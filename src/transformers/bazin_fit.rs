...
macro_const! {
    const DOC: &str = r#"
Transform BazinFit features to be more usable

The BazinFit feature extractor returns the following features:
...
- ln1p_bazin_fit_reduced_chi2 - transformed to be less spread
  ln(1 + reduced_chi2)
"#;
}
...
impl<T> Default for BazinFitTransformer<T>
where
    T: Float,
{
    fn default() -> Self {
...
impl<T> TransformerPropsTrait for BazinFitTransformer<T>
where
    T: Float,
{
    #[inline]
    ...
    fn names(&self, _names: &[&str]) -> Vec<String> {
        vec![
            "bazin_fit_mag_full_amplitude".into(),
            "bazin_fit_baseline_amplitude_ratio".into(),
            "bazin_fit_rise_time".into(),
            "bazin_fit_fall_time".into(),
            "ln1p_bazin_fit_reduced_chi2".into(),
        ]
    }
    ...
    fn descriptions(&self, _desc: &[&str]) -> Vec<String> {
        vec![
            format!(
                "Full amplitude of Bazin fit in magnitudes, zp={:.2} (zp - 2.5 log10(2A))",
                self.mag_zp
            ),
            "baseline-to-amplitude ratio of the Bazin function (B / A)".into(),
            "rise time of the Bazin function (tau_rise)".into(),
            "fall time of the Bazin function (tau_fall)".into(),
            "natural logarithm of unity plus Bazin fit quality (ln(1 + reduced_chi2))".into(),
        ]
    }
}

impl<T> TransformerTrait<T> for BazinFitTransformer<T>
where
    T: Float,
{
    fn transform(&self, x: Vec<T>) -> Vec<T> {
        let [amplitude, baseline, _reference_time, rise_time, fall_time, reduced_chi2]: [T;
            INPUT_FEATURE_SIZE] = match x.try_into() {
            Ok(a) => a,
            Err(x) => panic!(
                "BazinFitTransformer: expected {} features, found {}",
                INPUT_FEATURE_SIZE,
                x.len()
            ),
        };
...
mod tests {
    ...
    check_transformer!(BazinFitTransformer<f32>);
}
