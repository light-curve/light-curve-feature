...
mod tests {
    ...
    fn test_jacobian_internal_to_external() {
        ...
        let internal_pos = [1.0, 2.0, 3.0, 4.0];
        let jac = LinexpFit::jacobian_internal_to_external(&norm_data, &internal_pos);
        assert_relative_eq!(jac[0], m_std, epsilon = 1e-10); // A: sign(1) * m_std
        assert_relative_eq!(jac[1], t_std, epsilon = 1e-10); // t0: t_std
        assert_relative_eq!(jac[2], t_std, epsilon = 1e-10); // tau: sign(3) * t_std
        assert_relative_eq!(jac[3], m_std, epsilon = 1e-10); // B: m_std

        // Test with negative internal values (sign matters for abs() params)
        let internal_neg = [-1.0, 2.0, -3.0, 4.0];
        let jac = LinexpFit::jacobian_internal_to_external(&norm_data, &internal_neg);
        assert_relative_eq!(jac[0], -m_std, epsilon = 1e-10); // A: sign(-1) * m_std
        assert_relative_eq!(jac[1], t_std, epsilon = 1e-10); // t0: t_std (no abs)
        assert_relative_eq!(jac[2], -t_std, epsilon = 1e-10); // tau: sign(-3) * t_std
        assert_relative_eq!(jac[3], m_std, epsilon = 1e-10); // B: m_std (no abs)
    }
    ...
    fn test_internal_to_dimensionless_derivatives() {
        ...
        for i in 0..NPARAMS {
            ...
            let result = LinexpFit::internal_to_dimensionless(&hyper_param);
            let derivative = result[i][1];

            // Expected: sign(x) for params with abs(), 1.0 otherwise
            let expected = match i {
                0 | 2 => internal[i].signum(), // A, tau use abs()
                _ => 1.0,                      // t0, B are identity
            };
